...
interface TaihekiSelectionProps {
  onSelect: (mode: 'guide' | 'quick') => void;
}
      ...
      <div className="max-w-3xl mx-auto grid gap-6 sm:grid-cols-2">
        {options.map((option) => (
          <div key={option.id} className="relative">
            ...
            <div className="bg-white rounded-card p-6 shadow-z2 hover:shadow-z3 transition-all cursor-pointer h-full border-2 hover:border-green-500 group">
              <div className="text-center space-y-4">
                {/* Icon */}
                ...
                {/* Content */}
                <div>
                  <h3 className="text-h4-mobile font-heading text-light-fg mb-2">
                    {option.title}
                  </h3>
                  <p className="text-body-s-mobile text-light-fg-muted mb-4 leading-relaxed">
                    {option.description}
                  </p>
                ...
                {/* Button */}
                <Button
                  onClick={() => handleSelection(option.id as 'guide' | 'quick')}
                  disabled={isLoading}
