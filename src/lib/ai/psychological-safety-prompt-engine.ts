/**
 * Psychological Safety Prompt Engine
 * Implements Carl Rogers' 3 conditions for effective counseling:
 * 1. 自己一致 (Self-Congruence)
 * 2. 無条件の肯定的尊重 (Unconditional Positive Regard)  
 * 3. 共感的理解 (Empathetic Understanding)
 */
...
export class PsychologicalSafetyPromptEngine {
  private currentStage: ConversationStage['stage'] = 'warmup';
  private questionCount = 0;
  private lastSafetyScore: number = PROMPT_ENGINE_CONFIG.INITIAL_SAFETY_SCORE; // Start optimistic
  ...
  /**
   * Generate Rogers-compliant prompt with psychological safety considerations
   */
  public generatePsychologicalSafetyPrompt(
    messages: ChatMessage[] = []
  ): PsychologicalSafetyPrompt {
    // Analyze conversation stage progression
    this.updateConversationStage(messages);
    
    // Calculate current psychological safety score
    const safetyScore = this.calculateSafetyScore(messages);
    this.lastSafetyScore = safetyScore.overall;

    // Determine optimal question type based on stage and safety
    const questionType = this.selectQuestionType(safetyScore);

    // Get stage configuration
    const stage = this.getCurrentStageConfig();

    // Generate Rogers-compliant system prompt
    const systemPrompt = this.buildRogersCompliantPrompt(stage, safetyScore, questionType);
  ...
  private calculateSafetyScore(messages: ChatMessage[]): PsychologicalSafetyScore {
    ...
    // Analyze user response for safety indicators
    const analysis = this.analyzeUserResponse(lastUserMessage.content);
    
    // Calculate Rogers conditions compliance
  ...
  private analyzeUserResponse(content: string): UserResponseAnalysis {
    ...
    const engagement_level = Math.min(1.0, (wordCount / ENGAGEMENT_SCORING.WORD_COUNT_DIVISOR) + (positive_language.length * PROMPT_ENGINE_CONFIG.ENGAGEMENT_BOOST));
    ...
    const emotionalWords = [...positive_language, ...negative_language];
  ...
  private calculateRogersConditions(analysis: Partial<UserResponseAnalysis>): RogersConditions {
    ...
    // Self-Congruence (AI authenticity)
    const authenticity = safetyIndicators.engagement_level;
    const transparency = contentAnalysis.word_count > PROMPT_ENGINE_CONFIG.TRANSPARENCY_THRESHOLD ? ROGERS_SCORING_VALUES.HIGH_AUTHENTICITY : ROGERS_SCORING_VALUES.LOW_AUTHENTICITY;
    const limitations_acknowledged = true; // AI should always acknowledge limitations

    // Unconditional Positive Regard (acceptance)
    const acceptance_level = safetyIndicators.emotional_expression === 'resistant' ? ROGERS_SCORING_VALUES.LOW_ACCEPTANCE : ROGERS_SCORING_VALUES.HIGH_ACCEPTANCE;
    const non_judgmental_language = safetyIndicators.negative_language.length < PROMPT_ENGINE_CONFIG.RESISTANCE_THRESHOLD;
    const validation_present = safetyIndicators.positive_language.length > 0;

    // Empathic Understanding (comprehension)
    const understanding_level = Math.min(PROMPT_ENGINE_CONFIG.UNDERSTANDING_CAP, safetyIndicators.engagement_level + PROMPT_ENGINE_CONFIG.UNDERSTANDING_BOOST);
    const reflection_accuracy = contentAnalysis.sentiment !== 'negative' ? ROGERS_SCORING_VALUES.HIGH_REFLECTION : ROGERS_SCORING_VALUES.LOW_REFLECTION;
    const clarification_requested = contentAnalysis.word_count < PROMPT_ENGINE_CONFIG.TRANSPARENCY_THRESHOLD;
  ...
  private selectQuestionType(safetyScore: PsychologicalSafetyScore): 'choice' | 'open' | 'hybrid' {
    // Force choice questions if safety is low
  ...
  /**
   * Build Rogers-compliant system prompt
   */
  private buildRogersCompliantPrompt(
...
推奨語: 感じる、思う、考える、大切、理解、受け入れる`;
  ...
  /**
   * Generate recovery actions for low safety scores
   */
  private generateRecoveryActions(safetyScore: PsychologicalSafetyScore): SafetyRecoveryAction[] {
    ...
    if (!safetyScore.components.unconditionalPositiveRegard.non_judgmental_language) {
      actions.push({
        trigger: 'negative_language',
        action: 'privacy_reminder',
        message: 'このお話は完全に秘密が守られますので、安心してお聞かせください。'
      });
    }
  ...
  // Helper methods for initial states and defaults
